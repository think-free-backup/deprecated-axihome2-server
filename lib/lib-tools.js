...
exports.loadScene = function(name, callback){
	...
	try{
		...
		for (var idx in scene.actions){
			...
			if (item.delay === 0)
				exports.callDevice(item.name, item.processor, item.deviceType, item.device, item.actuator, item.value);
			else
				delayCall(item);
		}
...
exports.callDevice = function(name, processor, deviceType, device, actuator, value, callback){

	try{

	    var poolerParams = getParamsForDevice(processor, name);
	    var proc = require("../processors/" + processor + "/main.js");
	...
	function getParamsForDevice(processor, name){
